...
veh_list = ['v0_w2e'] # IDs of vehicles of interest
...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if step == 1:
            ...
            traci.vehicle.changeSublane('v0_w2e', 5)
        if (step%2) == 0:
            if step > 20 and step < 50:
                traci.vehicle.changeSublane('v0_w2e', np.random.normal(0,0.4))
            else:
                traci.vehicle.changeSublane('v0_w2e', np.random.normal(0,0.2))
        if step == 18:
            traci.vehicle.slowDown('v0_w2e', 2.0, 28)
        #if step > 60 and step < 80:
        #    traci.vehicle.changeLane('v0_w2e', 1, 1)
        ...
        step += 1
    with open(r'data/all_veh_data_tune2.yaml','w') as file:
...
if __name__ == "__main__":
    ...
    traci.start([sumoBinary, "-c", "structure/intersection_1v_w2e.sumocfg", "--lateral-resolution=0.2"])
    run()